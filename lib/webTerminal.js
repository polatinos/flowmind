 ...
 *
 * Scope — read this before changing anything here. The page shows a chat box,
 * but the chat *is* the assistant: whoever reaches this port with the token
 * can switch every device, read and write memories, and create, change or
 * delete flows. Only the API keys stay out (those live in the Homey settings
 * page). So this is not "chat only, no secrets" — it is full house control
 * over plain HTTP on the LAN, and the token is a house key.
 *
 * What guards it:
 * - a random token (settings key `webTerminalToken`), compared in constant
 *   time; the link in the settings page carries it once in the query string,
 *   and `GET /` immediately swaps it for an HttpOnly cookie so page scripts
 *   never see it and it no longer sits in localStorage;
 * - a `Host` check, so a public hostname pointed at this IP (DNS rebinding)
 *   is refused;
 * - `SameSite=Strict` plus a required JSON content type, so another site
 *   cannot make a browser post a turn on the user's behalf;
 * - a rate limit on `POST /api/chat`, so a token holder cannot burn API
 *   credits or the Homey's memory in a loop;
 * - a sanitised chat body: the page may pick a known provider, but the model,
 *   the step budget and the flow flag come from settings, never from the LAN.
 *
 * Still true after all that: the traffic is plain HTTP, so anyone who can
 * watch the network sees the conversation.
 */
...
const COOKIE_NAME = 'flowmind_wt';
const COOKIE_MAX_AGE = 60 * 60 * 24 * 30; // 30 days
const MAX_MESSAGES = 40; // what the page keeps anyway

// Chat turns cost API credits and memory on a device that has little of
// either, so the untrusted port gets a ceiling: per client and overall.
const RATE_WINDOW_MS = 60 * 1000;
const RATE_MAX_PER_CLIENT = 8;
const RATE_MAX_TOTAL = 20;
const RATE_MAX_CLIENTS = 64;
...
class WebTerminal {
  constructor(app) {
    ...
    this.server = null;
    this.lastError = null;
    this._chatHits = new Map(); // remote address -> timestamps within the window
  }
  ...
  /**
   * The cookie is how a browser authenticates after the one-time handoff; the
   * header stays for scripts on the LAN that hold the token themselves. The
   * query string authenticates nothing beyond `GET /`, which mints the cookie.
   */
  _authorized(req) {
    return (
      this._tokenMatches(this._cookieToken(req))
      || this._tokenMatches(req.headers['x-flowmind-token'])
    );
  }

  /**
   * Refuse a request whose `Host` is a name that could be registered on the
   * public DNS: that is the DNS-rebinding shape, where a page the user opens
   * resolves to this LAN address and then talks to this server as same-origin.
   * IP literals, `localhost`, single-label names and the private suffixes
   * cannot be pointed here from the outside, so they pass.
   */
  _hostAllowed(req) {
    const raw = String(req.headers.host || '');
    if (!raw) return false;
    const hostPart = raw.startsWith('[') ? raw.slice(0, raw.indexOf(']') + 1) : raw.split(':')[0];
    const name = hostPart.replace(/^\[|\]$/g, '').toLowerCase();
    if (!name) return false;
    if (net.isIP(name)) return true;
    if (name === 'localhost') return true;
    if (!name.includes('.')) return true;
    return /\.(local|lan|home|internal|homey)$/.test(name);
  }

  /**
   * Sliding window over the last minute. Returns 0 when the turn may run, or
   * the number of seconds to wait when it may not.
   */
  _chatRateDelay(req) {
    const now = Date.now();
    const client = (req.socket && req.socket.remoteAddress) || 'unknown';

    for (const [key, hits] of this._chatHits) {
      const kept = hits.filter((t) => now - t < RATE_WINDOW_MS);
      if (kept.length) this._chatHits.set(key, kept);
      else this._chatHits.delete(key);
    }

    const mine = this._chatHits.get(client) || [];
    let total = 0;
    for (const hits of this._chatHits.values()) total += hits.length;

    const overClient = mine.length >= RATE_MAX_PER_CLIENT;
    const overAll = total >= RATE_MAX_TOTAL;
    if (overClient || overAll) {
      let oldest = now;
      if (overClient) oldest = mine[0];
      else for (const hits of this._chatHits.values()) oldest = Math.min(oldest, hits[0]);
      return Math.max(1, Math.ceil((RATE_WINDOW_MS - (now - oldest)) / 1000));
    }
    // A full table means someone is cycling addresses; hold everyone briefly
    // rather than letting the map grow without bound.
    if (!this._chatHits.has(client) && this._chatHits.size >= RATE_MAX_CLIENTS) {
      return Math.ceil(RATE_WINDOW_MS / 1000);
    }

    mine.push(now);
    this._chatHits.set(client, mine);
    return 0;
  }

  /**
   * Keep only what this port is allowed to decide. The provider dropdown is a
   * real feature of the page, so a known provider id is accepted; everything
   * else that `startChat` understands (model, maxSteps, fromFlow) is settings'
   * business and is dropped here.
   */
  _chatBody(body) {
    ...
    if (
      ...
      && Object.prototype.hasOwnProperty.call(PROVIDERS, body.provider)
    ) {
  ...
  async _handle(req, res) {
    ...
    if (!this._hostAllowed(req)) {
      this._json(res, 403, { error: 'Reach this terminal by its address on the local network.' });
      return;
    }

    // The page itself. A valid ?token= is the one-time handoff from the
    // settings link: it is swapped for an HttpOnly cookie and then gone.
    if (req.method === 'GET' && path === '/') {
      const handoff = this._tokenMatches(url.searchParams.get('token'));
      if (!handoff && !this._authorized(req)) {
      ...
      if (handoff) {
        headers['Set-Cookie'] = `${COOKIE_NAME}=${encodeURIComponent(url.searchParams.get('token'))}`
    ...
    // A form post from another site cannot set this content type, so together
    // with the SameSite cookie it keeps cross-site requests out.
    if (req.method === 'POST') {
      const type = String(req.headers['content-type'] || '').split(';')[0].trim().toLowerCase();
      if (type !== 'application/json') {
        this._json(res, 415, { error: 'Expected application/json' });
        return;
      }
    }
    ...
    if (req.method === 'POST' && path === '/api/chat') {
      const wait = this._chatRateDelay(req);
      if (wait) {
        this._json(res, 429, { error: `Too many requests — try again in ${wait}s.` }, { 'Retry-After': String(wait) });
        return;
      }
      const body = await this._readBody(req);
      const started = await this.app.startChat(this._chatBody(body));
...
(function () {
  ...
  var T = {
    ...
    free: NL ? 'gratis' : 'free',
    expired: NL
      ? 'Geen toegang \\u2014 open de terminal opnieuw via de link in de FlowMind-instellingen.'
      : 'No access \\u2014 reopen the terminal through the link in the FlowMind settings.',
    timeout: NL ? 'Duurde langer dan 5 minuten \\u2014 probeer opnieuw.' : 'Took longer than 5 minutes \\u2014 try again.'
  };

  // The token came in through the URL and the server has already swapped it
  // for an HttpOnly cookie, so clear it from the address bar and delete any
  // copy an older version of this page left behind in localStorage. Nothing
  // here can read the token any more, which is the point.
  ...
  function api(method, path, body) {
    return fetch(path, {
      ...
      if (!r.ok) return r.json().catch(function () { return {}; }).then(function (j) {
        throw new Error(r.status === 403 ? T.expired : (j.error || ('HTTP ' + r.status)));
      });
      return r.json();
    });
  }

...
class HomeyContext {
  ...
  _setMemories(list) {
    ...
    this.homey.settings.set('aiMemories', list.slice(-60));
  }

  async saveMemory({ text } = {}) {
    const value = typeof text === 'string' ? text.trim() : '';
    if (!value) return { error: 'No text provided.' };
    if (value.length > 500) return { error: 'Memory too long (max 500 characters). Summarise it.' };
    ...
    if (existing) return { ok: true, id: existing.id, note: 'Already saved.' };

  ...
  async init() {
    ...
    if (localKey) {
      try {
        ...
        // revoked key still hands back a client, and every call afterwards
        // fails with "Invalid Token" while the settings page cheerfully
        // reports the key as working. /session/me needs no scopes (verified
        // in the API spec), so it fails only when the token itself is refused.
        await this.api.sessions.getSessionMe();
        this.apiMode = 'local';
        this.localFailedAt = 0;
        return this.api;
      } catch (err) {
        ...
        // Drop the half-built local client; the app-mode one replaces it below.

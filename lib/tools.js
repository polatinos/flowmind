...
const TOOL_DEFINITIONS = [
  ...
  },
  {
    name: 'check_flows',
    description:
      'Diagnose every flow and report the ones that are silently dead. Finds cards pointing at ' +
      'devices that no longer exist, cards belonging to an uninstalled app, and — in advanced ' +
      'flows — how many later cards can no longer be reached because an earlier card fails. ' +
      'Homey itself does NOT flag most of this: a restart_app card whose target app was ' +
      'uninstalled still counts as healthy, so the flow reports broken=false while doing nothing. ' +
      'Read-only; it never changes a flow. Use it when the user asks whether their flows still ' +
      'work, after apps or devices have been removed, or when an automation stopped working for ' +
      'no clear reason.',
    input_schema: {
      type: 'object',
      properties: {
        flowId: {
          type: 'string',
          description: 'Only check this one flow. Omit to check every flow.',
        },
      },
    },
  },
